...
const encadrantsOnly = restrictTo('encadrant_entreprise', 'encadrant_universitaire');
const encadrantUniversitaireOnly = restrictTo('encadrant_universitaire');
const allRoles = restrictTo('etudiant', 'encadrant_entreprise', 'encadrant_universitaire');

// Middleware pour vérifier si l'utilisateur peut accéder au projet
...
console.log(project.student);
    ...
    res.status(500).json({
      success: false,
      message: 'Erreur lors de la vérification des droits projet '+error.message
    });
  }
};

// Middleware pour vérifier si l'étudiant est propriétaire du projet
